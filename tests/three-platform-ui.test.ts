...
it("preserves the legacy hero and adds the three platform section", async () => {
  ...
  const dailyBrief = await readFile("app/components/daily-brief.tsx", "utf8");
  const detailPage = await readFile("app/recommendations/[slug]/page.tsx", "utf8");
  ...
  expect(card).toContain("MonthlyRevenueCard");
  expect(dailyBrief).toContain("MonthlyRevenueCard");
  expect(detailPage).toContain("MonthlyRevenueCard");
});
